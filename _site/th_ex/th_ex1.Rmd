...
### 3.3.2. Distribution by Household Size
...
### 3.3.3. Distribution by 'Have Kids'

For *haveKids* it is a logical data which takes either TRUE or FALSE values. We see that majority of the participants do not have kids yet. This variable has similar details as the previous chart in *3.3.2.*, where *householdSize = 3* would also give an indication whether the participant has kids.
...
```
Based on the summary statistics calculated at *section 3.3.1*, the mean age of the participants is 39. As it is difficult to compare the discrete age, we will group them in interval of 5 years  from age 20 - 60 years old.


#### 3.3.4.1. Distribution by Age Groups
...
```
## Distribution by Interest Groups
...
When the bar chart based on the education level was plotted, we note that majority of the participants minimally has completed high school or colleges.  More than one fifth of them, have a bachelor's degree.

```{r}
participants_data %>% mutate(educationLevel = fct_infreq(educationLevel)) %>%
...
## 1. age and education level (to exclude)
...
ggplot(data = participants_data,
  ...
  coord_flip()
```
## Age spread of interest groups (to exclude)
...
```
## Joviality Index based on Age Group and Education Level
...
ggplot(data = participants_data,
  ...
  facet_grid(educationLevel ~.)
```
## Joviality Index based on Interest Group and Education Level
...
3. Little information for the variable *interestGroup*.
